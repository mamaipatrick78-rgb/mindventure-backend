...
// Re-checks is_admin from the DB on every request rather than trusting a JWT claim, so
// revoking admin access takes effect immediately without waiting for tokens to expire.
...
app.post('/api/withdrawals',auth,async(req,res)=>{
  ...
  if(!['BTC','ETH','USDT','SOL'].includes(d.asset))return res.status(400).json({error:'Unsupported asset'});
  ...
  // NOTE: this executes immediately on testnet, with no KYC/2FA/manual-review gate — that
  // gate must exist before this ever runs against mainnet funds. See custody/adapter.js.
  try{
