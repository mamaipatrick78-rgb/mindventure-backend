...
app.post('/api/deposit/address',auth,async(req,res)=>{const d=schema.parse(req.body);const w=await q('SELECT * FROM wallets WHERE user_id=$1 AND asset=$2 AND network=$3',[req.user.sub,d.asset,d.network]);if(w.rows[0])return res.json(w.rows[0]);const a=await custody().createDepositAddress({...d,userId:req.user.sub});const r=await q('INSERT INTO wallets(user_id,asset,network,address,provider_ref) VALUES($1,$2,$3,$4,$5) RETURNING *',[req.user.sub,d.asset,d.network,a.address,a.providerRef]);res.status(201).json(r.rows[0])});
